...
1. The checkout generation under `functions/v1/get-entitlements` route is not sandbox unless user's emails are under the `@freemius.com` domain.
...
3. If the app has been published, make sure to update all front-end URLs to point to the production URL instead of the local development URL.
