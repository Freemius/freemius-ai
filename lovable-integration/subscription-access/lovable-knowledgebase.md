...
1. When asked to Integrate with Freemius for the first time, follow the
   instructions by visiting the markdown file in the github repository.
2. As the shared `supabase/functions/_shared/freemius.ts` module is built, use
   that to protect any API route that needs to be behind the paywall.
3. In the same module we will have constants for the lovable front-end URL and
