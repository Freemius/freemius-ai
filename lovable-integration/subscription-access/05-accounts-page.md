Now we will create an accounts page where the user can see their subscription status and get a link to Freemius Customer Portal to manage their subscription.

1. Create a new page `/accounts` in the front-end, make sure the link is visible in the navigation menu.
2. The page will also make the same API supabase function call to `functions/v1/get-entitlements` to get the checkout session link and the entitlements.
...
7. If the browser fails to open the tab (due to popup blockers), then show the link to the user and ask them to open it manually.
   ...
   2. Do show a UI saying the link will expire in 5 mins. You can also add a timer counting down to when the link will expire.
   3. Hide the URL after 5 mins because it will expire and show the main button again.
8. If the user doesn't have an active entitlement, show the "Subscribe" button that redirects to the checkout page. The checkout URL is returned from the same `functions/v1/get-entitlements` API route.
